...
namespace DAL.FluentAPI
{
    public class UserFluentAPI : IEntityTypeConfiguration<DtoUser>
    {
        public void Configure(EntityTypeBuilder<DtoUser> builder)
        {
            ...
            builder.Property(x => x.Role).HasMaxLength(10).HasDefaultValue("customer");
            builder.Property(x => x.PasswordHash);
            // list cart by user
            builder.HasOne(x => x.Information).WithOne(x => x.User).HasForeignKey<DtoInformation>(x => x.UserId);
