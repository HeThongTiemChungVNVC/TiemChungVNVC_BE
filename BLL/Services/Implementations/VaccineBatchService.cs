	...
	public class VaccineBatchService : IVaccineBatchService
	{
		...
		public async Task<ApiResponse<string>> DeleteVaccineBatch(string idVaccineBatch)
		{
			try
			{
				...
				if (entity == null)
				{
					return ApiResponse<string>.ApiResponseFail("Lô vaccine này không tồn tại", "Lô vaccine này không tồn tại");
				}
		...
		public async Task<ApiResponse<VaccineBatchResponse>> GetVaccineBatch(string idVaccineBatch)
		{
			try
			{
				var entity = repository.GetAll().AsQueryable().Include(x => x.Supplier).Include(x => x.PriceVaccine).Include(x => x.Vaccine).FirstOrDefault(x => x.Id == idVaccineBatch && !x.IsDeleted);
				if (entity == null)
				{
